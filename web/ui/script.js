document.addEventListener("DOMContentLoaded", () => {
  ...
  // Exibir ou ocultar botões dependendo do perfil
...
function configurarModais(usuario) {
  ...
  btnFecharAtividade.onclick = () => {
    ...
    delete formAtividade.dataset.turmaId;
    turmaAtualId = null;
  };

  // Combinar window.onclick para ambos os modais
  ...
  formAtividade.onsubmit = async (e) => {
    ...
    const turmaId = formAtividade.dataset.turmaId || turmaAtualId;
    ...
    try {
      ...
      if (id) {
        // Atualizar
        res = await fetch(`http://localhost:3005/atividade/${id}`, {
          method: "PUT",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ nome }),
        });
        ...
        // Criar
        res = await fetch("http://localhost:3005/atividade", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            nome,
      ...
      carregarAtividades(turmaId, usuario);
    } catch (err) {
      alert(err.message);
    }
  };
...
async function carregarAtividades(turmaId, usuario) {
  try {
    const res = await fetch("http://localhost:3005/atividade");
    if (!res.ok) throw new Error("Erro ao buscar atividades");
    const atividades = await res.json();

    // Filtrar atividades da turma
...
async function visualizarAtividades(turmaId, nomeTurma) {
  ...
  // Exibir botão "Nova Atividade" para professores
  if (usuario.perfilId === 2) {
    document.getElementById("btnCadastrarAtividade").style.display = "inline-block";
    document.getElementById("btnCadastrarAtividade").onclick = () => abrirModalAtividade(turmaId);
  } else {
